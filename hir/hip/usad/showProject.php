...
switch ($qw) {
    case 0:
        $query = "SELECT distinct tblprojects.id, tblprojects.project,tblprojects.description, tblprojects.deadline,tblprojects.started,tblprojects.status,tblongoing_tasks.id, tblongoing_tasks.task,tblongoing_tasks.user,tblongoing_tasks.deadline,tblongoing_tasks.date_assigned,tblongoing_tasks.percentage,tblongoing_tasks.proj,tblusers.full_name
         FROM tblprojects
        ...
        break;
    case 1:
        // $qq= mysqli_query($con, "SELECT tblongoing_tasks.user FROM tblongoing_tasks where company='".$_SESSION['company']."'");
        // while($rd=mysqli_fetch_assoc($qq)){
        //     $sz=$rd['tblongoing_tasks.user'];
        //     $za = implode(',', $sz);
        //     // echo $za; 
        //     // $cs=count($za);
        //     // for($i=0; $i<$cs; $i++)
        // {
         $query = "SELECT distinct (tblprojects.project),tblprojects.id, tblprojects.description, tblprojects.deadline,tblprojects.started,tblprojects.status,tblongoing_tasks.id, tblongoing_tasks.task,tblongoing_tasks.user,tblongoing_tasks.deadline,tblongoing_tasks.date_assigned,tblongoing_tasks.percentage,tblongoing_tasks.proj,tblusers.full_name
                    FROM tblprojects
                    LEFT JOIN tblongoing_tasks
                    ON tblprojects.id = tblongoing_tasks.proj
                    LEFT JOIN tblusers 
                    ON tblusers.id = tblongoing_tasks.user
                    where tblprojects.company='".$_SESSION['company']."' 
                     AND tblongoing_tasks.user LIKE '%,' 
                    
                    ";
                    // (',' + RTRIM(tblongoing_tasks.user)+',') LIKE '%,' + @search + ',%'  
        // AND count(implode(',', tblongoing_tasks.user)) > 1)
                    // AND count(tblongoing_tasks.user) > 1)
