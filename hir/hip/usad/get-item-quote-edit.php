...
$event_id = $_GET['c'];
			...
			//Get expiry date, creation date and quotation status from database
			$connn =  mysqli_connect('localhost', 'merimeve_event', 'user@event', 'merimeve_event') or die("connection failed" . mysqli_error()); 

			
			$ref=mysqli_query($connn,"SELECT * FROM event_quotation WHERE event_id='$event_id'");
			while($rows = mysqli_fetch_array($ref)){
				$cdate=$rows['creation_date'];
				$edate=$rows['expiry_date'];
				$qstat=$rows['quotation_status'];
			}
...
while($row = mysqli_fetch_array($result)) { 
  ...
  echo "<td><input type='number' name='txtPrice' value='".$row["lease_charges"] . "' class='form-control' required>";
  echo "<input type='hidden' name='creation_date' value='".$cdate."' class='form-control'>";
  echo "<input type='hidden' name='expiry_date' value='".$edate."' class='form-control'>";
  echo "<input type='hidden' name='quotation_status' value='".$qstat . "' class='form-control'></td>";
