...
//======================================//
 ...
 // Payment for event quotation
 if(isset($_POST['submit_payment'])){
 	$even_id = $_POST['event_id'];
 	$check_events=mysqli_query($con,"SELECT * FROM event_quotation_status WHERE event_id='$event_id'");
     if(mysqli_num_rows($check_events) < 1){
    $set_status=mysqli_query($con," INSERT INTO `event_quotation_status`(`event_id`, `quotation_status`, `company`) VALUES ('$event_id',1, '$compp')");
 if($set_status==true){

    echo '<script> alert("Paid successfully"); window.history.back();</script>';	 
 }   
 }
...
//							 
                              ...
                              if($y > 0){
                                    ...
                                    echo '<a href="print-quotation?id='.$event_id.'" target="_blank" class="btn btn-primary btn-sm form-group" style="margin-top:30px;margin-bottom:30px;float:center;" ><i class="fas fa-print"></i>&nbsp;Print quotation</a>&nbsp;&nbsp;&nbsp';
								  echo '<a class="btn btn-primary btn-sm form-group" style="margin-top:30px;margin-bottom:30px; data-toggle="tab" href="#store"><i class="fas fa-file-invoice"></i>&nbsp;Store requisition form</a>
                        ';
...
-->
                  ...
                  ?>
										...
										<section class="content">
											<div class="box">
												...
												<div class="box">
													...
													$sedey=mysqli_query($con,"SELECT * FROM event_quotation_status WHERE event_id='$event_id'");
													$no_sedey=mysqli_num_rows($sedey);
													if($no_sedey > 0){?>
                                            ...
                                            $task=0;
                                            $date=date("Y-m-d");
                                            $query78  = "SELECT * FROM event_quotation WHERE company='$compp' AND event_id='$event_id'";
                                            $result78 = mysqli_query($con, $query78);
                                                while ($row = mysqli_fetch_array($result78))
                                                  { 
                                                      $eventt_id = $row['id'];
                                                      $event_name=$row['event_name'];
                                                      $cust_name=$row['customer_name'];
                                          ...
                                          ?>
                                            ...
                                            ?>
														...
														<em>
															<center><a href="#quotation" class="btn btn-primary btn-sm" data-toggle="tab">Pay items in invoice to generate a store requisition form! </a></center>
														</em>
								...
								<form method="post" action="view_event.php?id=<?php echo $event_id;?>">
										<div class="modal-dialog" role="document">
											<div class="modal-content">
												<div class="modal-header">
													...
													<strong>
														<center>
															<h4 class="modal-title text-primary" style="font-family:family!important;" id="modalLabel">Make Payments</h4>
														</center>
													</strong>
												</div>
												<div class="modal-body">
													<label style="margin-top: 20px;">Amount Owed</label>
													<div class="input-group mb-2">
														<span class="input-group-addon bg-white border-right-0 px-4">Ksh</span>
														<input id="amountOwed" type="text" class="form-control bg-white border-left-0 " name="amount_owed" value="<?php echo $overall_total; ?>" readonly>
													</div>
													<label style="margin-top: 20px;">Method of payment</label>
													<div class="input-group mb-2">
														<span class="input-group-addon bg-white border-right-0 px-4"><i class="fas fa-calendar"></i></span>
														<select name="pay_method" class="payMethod form-control bg-white border-left-0">
															<option disabled selected>Select </option>
															<option value="partial">Partial Payment</option>
															<option value="full">Paid in Full</option>
														</select>
													</div>
													<!---If partial payments display this-->
													<div style="display:none;" class="partial">
														<label style="margin-top: 20px;">Enter Amount Paid <span class="text-success" style="font-family:fangsong!important;"><strong><em>(Amount cannot exceed amount owed!)</em></strong></span></label>
														<div class="input-group mb-2">
															<span class="input-group-addon bg-white border-right-0 px-4">Ksh</span>
															<input type="hidden" name="event_id" value="<?php echo $event_id;?>">
															<input id="amountPaid" onkeyup="return myFunctionalz()" type="number" class="form-control bg-white border-left-0 " name="amount_paid" placeholder="Enter Amount Paid Now" min="1" max="<?php echo $overall_total; ?>" oninput="validity.valid||(value='');">
														</div>


													</div>
													<!----if full payments display this-->
													<div style="display:none;" class="full">
														<label style="margin-top: 20px;">Enter Amount Paid</label>
														<div class="input-group mb-2">
															<span class="input-group-addon bg-white border-right-0 px-4">Ksh</span>
															<input type="number"  class="form-control bg-white border-left-0 " value="<?php echo $overall_total; ?>" placeholder="Enter Amount Paid Now" readonly>
														</div>
													</div>
												...
												<div class="modal-footer">
													<button type="submit" class="btn btn-success" name="submit_payment">Submit</button>&nbsp;
                              ...
                              ?>
												...
												<div class="box-body table-responsive">
													<div class="form-row">
														...
														<button class="btn btn-info btn-sm form-group" style="margin-top:10px;margin-bottom:30px;" data-toggle="modal" data-target="#flipFlop">Pay <i class="fas fa-file-invoice"></i></button>
														<?php }else{ ?>
										...
										<div class="modal-dialog">
											...
											<div class="modal-content">
												<div class="modal-header">
													...
													<h4 class="modal-title text-primary" style="font-family:fangsong!important;">
														<center>Make payments</center>
													</h4>
												</div>
												<div class="modal-body">
...
-->
	...
	<script type="text/javascript">
		function myFunctionalz {

			var x = document.getElementById("amountOwed").value;
			var y = document.getElementById("amountPaid").value;
			var totalz = x - y;


			document.getElementById("balanceit").value = totalz;
		}
