...
//Get this user's ID
$id=$_GET['id'];//rReceipients chat
// $user_id=$_SESSION['userid'];

//
$squeri = mysqli_query($con, "select * from tblusers where company='".$_SESSION['company']."' AND id='$id'");
                                            while($rowi = mysqli_fetch_array($squeri))
                                            {
												$rec_name =$rowi['full_name'];
											}

    
    if(isset($_POST['okayadmin2'])){
        ...
        $result = mysqli_query($con,"INSERT INTO singlefaq (user,message,ask_date,username,company,sender_id,rec_id) VALUES ('$identity','$faqmessage','$todate','$name','$company','$identity', '$id')");
       if($result==true){
    ...
    $_SESSION['sent'] = 1;
		 ...
		 $erd =mysqli_error($con);
             echo '<script type="text/javascript">'; 
    echo 'alert("Error during submission: '."$erd".'");'; 
    echo '</script>';
       }
...
//Might be a directory issue??
	 ...
	 if($z){
		 $sql=mysqli_query($con,"INSERT INTO tbltasksdocuments(doc_name, size,company) VALUES ('$uploaded_file', '0','$companyz' )");
...
<body class="skin-black">
							...
							<div class="container overflow-auto col-lg-8 col-md-8 col-sm-8" style="height: 550px;">
								<section class="msger">
									<header class="msger-header">
										<div class="">
											<i class="fas fa-user fa-2x " style="color:#00ff55;font-size: 13px;"></i>&nbsp;&nbsp; <?php echo strtoupper($rec_name); ?>
										</div>
									</header>
  ...
  $sql090=mysqli_query($con,"SELECT * FROM singlefaq WHERE (sender_id='$identity' AND rec_id='$id') OR (rec_id='$identity' AND sender_id='$id')");
    while($ros =mysqli_fetch_assoc($sql090)){
	 ...
	 $send_adress=$ros['sender_id'];
      $usersession = $_SESSION['userid'];


	 if($send_adress==$identity){
    ...
    ?>
									...
									<form class="msger-inputarea" method="POST" action="">
										...
										<div class="form-row">
											...
											<button type="submit" class="btn btn-success form-group" name="okayadmin2">Send</button>
										</div>
									</form>
								</section>
							</div>
							...
							<div class="col-lg-4 col-sm-12 col-md-4" style="height:550px!important; overflow: auto!important;">
								<section class="msger">
									...
									<div id="chatscreen" class="msger-chat">
										...
										<div class="form-row">
											<div clas="mb-3"><a  href="livechat.php"><i class="fas fa-users fa-2x" style="color:#ffad33;font-size: 15px"></i>&nbsp; &nbsp;Group chat</a></div><br>
											<?php
									$squery = mysqli_query($con, "select * from tblusers where company='".$_SESSION['company']."' AND id!='$identity' AND id!='$id' ORDER BY id DESC");
                                            while($row = mysqli_fetch_array($squery))
                                            {
												$new_id=$row['id'];
												?>
								
											<div class="mt-2 form-group"><i class="fas fa-user fa-2x" style="color:#00ff55;font-size: 13px;"></i>&nbsp;&nbsp;<span style="font-size: 15px"><a onclick="readChat(<?php echo $new_id; ?> )" href="single_chat.php?id=<?php echo $new_id; ?>" onClick="updateId('<?php echo $new_id; ?>')"><?php echo $row['full_name']; ?></a></span>
									<?php
						$q = mysqli_query($con,"SELECT * FROM singlefaq where rec_id ='$identity' and sender_id='$new_id' and reply IS NULL");
                $num_rows = mysqli_num_rows($q);
                if($num_rows>0){
               echo $new_sms = 
                '<span class="badge" style="float:right;background-color:#00802b;"><a style="color:#fff !important;" href="single_chat.php?id='.$new_id.'" onClick="readChat('.$row['id'].')">'.$num_rows.'</a></span>';
              }else{
               echo $new_sms = '';
              }  
											?>
