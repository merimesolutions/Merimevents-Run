...
if(isset($_POST['send_email'])){
	  // $img = mysqli_query($con,"SELECT profile_img staff WHERE company = '".$_SESSION['company']."'");
	        $to = $_POST['email'];
	        $fromer = "events@merimevents-run.com";
	        //from pass= n{#HnYKDIqTD
	        $message = $_POST['message'];
...
try {
  ...
  // $mail->SMTPDebug = 4;                     //Enable verbose debug output
    ...
    $mail->Host       = 'mail.merimevents-run.com';                     //Set the SMTP server to send through
    $mail->SMTPAuth   = true; 
    $mail->Username   = 'info@merimevents-run.com';                     //SMTP username
    $mail->Password   = 'infor@everun';                               //SMTP password
    $mail->SMTPSecure = 'tls';            //Enable implicit TLS encryption
    $mail->Port       = 25;  //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

    //Recipients
    $mail->setFrom($fromer, 'Merime Events Run');
   foreach($ex_to as $value){
   $mail->addAddress($value);     //Add a recipient
    $mail->addReplyTo($fromer, 'Merimeve Events Run');
     
    //Attachments
    // $mail->addAttachment($cv2send,$tmp_name);         //Add attachments
    // $mail->addAttachment($business_plan_send, $tmp_name_plan);    //Optional name
    //Content
...
<script type="text/javascript" src="../../assets/sweetalert/sweetalert2.all.js"></script>
 ...
 //    $data = mysqli_fetch_assoc($get_customer);
				...
				<div class="tab-content">
					<div id="home" class="tab-pane fade in active" style="border-bottom: 1px solid #ddd;border-left: 1px solid #ddd;border-right: 1px solid #ddd;padding:12px;">
                      ...
                      $emaild[]= $users['email'];
							...
							<div class="col-lg-8">
						<form class="main form-group p-4 bg-light" action="text.php" id="form_email" method="POST" >
							<div class="btn-txt"></div>
						<!-- 	<div class="form-group  ">
							  <label>From.</label>
							  <input type="text" class="form-control" name="from_email" id="customerMail_from" placeholder="Email address" aria-label="" required aria-describedby="basic-addon1">
				    ...
				    <div class="form-group">
				     ...
				     <textarea class="form-control" placeholder="Write a message" name="message" id="summernote" style="height: 200px"></textarea>
				    </div>
				<br>
						<div class="form-group">
							  
						    <button type="submit" class="btn btn-primary" id="send_email" name="send_email"    > <i class="loading-icon fa fa-spinner fa-spin hide"></i> Send Now</button>
   ...
   //        // $(".btn-txt").text("Sending emails...");
   //         setTimeout(function(){  
   //          $(".loading-icon").addClass("hide");
   //          $("#send_email").attr("disabled", false);
   //          // $(".btn-txt").text("Mails Sent");
   //           }, 6000);
