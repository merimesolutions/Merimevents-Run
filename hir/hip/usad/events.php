...
if(isset($_POST['add_event'])){
    ...
    // }
    $tk=mysqli_query($con,"SELECT * FROM tblcustomers WHERE customer_name='".$customer_name."' AND company='".$_SESSION['company']."'");
    if($tk){
    ...
    if($squery){
        ...
        echo '<script>alert("Event Added successfully");window.location="quotation_type?id='.$last_id.'"</script>';
    }else{
...
//Comprehensive Customers
  ...
  $sql_u =mysqli_query($con, "SELECT * FROM tblcustomers WHERE identity = '$identity' and company='$company' ");
        if($sql_u){
            ...
            if($numbb > 0){
            echo '<script>alert("This customer record already exists in the system!");window.location="events";</script>';
    }else{
          ...
          if($sql){
                echo '<script>alert("Customer Registered succesfully");window.location="events";</script>';
          } else {
        
        echo '<script>alert("An error occured during submission");window.location="events";</script>';
          }
 ...
 if(isset($_POST['btn_add_cutomer'])){
     ...
     if($number_pe > 0){
         echo '<script>alert("This customer record already exists!!");window.location="events";</script>';
     }else{
         ...
         if($sl){
             ...
             echo'<script>alert(" '."$errp".'");window.location="events";</script>'; 
         }
     }
 }
?>
...
<body class="skin-black">
	...
	<div class="wrapper row-offcanvas row-offcanvas-left">
		...
		<aside class="right-side">
			<section class="content-header">
				...
				<a class="float-left" role="button" data-toggle="modal" data-target="#myModalzz"><i class="fa fa-plus"></i> Add event</a>&nbsp;&nbsp;&nbsp;
				<a class="" href="archived" title="Archived events" style="margin-bottom:5px;"><i class="fa fa-archive"></i> Archived events</a>
			</section>
			<!--=======================================================================================================================-->
  ...
  z-index: 1;">
															...
															<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton" style="padding:10px;">
																<input type="submit" name="archive_event" class="dropdown-item" style="background-color:transparent ; border:none;" value="Archive event">
																<input type="submit" name="delete_event" class="dropdown-item" style="margin-top:10px !important;background-color:transparent ; border:none;" value="Delete event">
															</div>
														</div>
														<thead>
															<tr>
                                            ...
                                            $task=0;
                                            $date=date("Y-m-d");
                                            $query  = "SELECT * FROM tblevents WHERE company='$compp' and (del=0 or del IS NULL) ORDER BY id DESC";
                                            ...
                                            echo '
                                            <tr>
                                            <td><input type="checkbox" name="events[]" value="'.$event_id.'"></td>
                                                <td>'.$c++.'</td> 
                                                <td>'.ucwords($row['event_name']).'</td>
                                                <td>'.ucwords($row['customer_name']).'</td>  
                                                <td>'.$row['start_date'].'</td>
                                                <td>'.ucwords($row['end_date']).'</td> 
                                                  <td><center><a data-target="#taskass'.$row['id'].'" data-toggle="modal" class=""><i class="fas fa-users"></i></a></center></td>
                                                  
                                                <td><center><a href="quotation_type?id='.$row['id'].' " class="" ><img src="../../images/icons/doc_edit.png" title="Edit this record" class="iconb"></a></center></td>
                                              
                                                
                                                <td><center><a href="quotation_type?id='.$row['id'].'" ><img src="../../images/icons/eye.png" title="Edit this record" class="iconb"></a></center></td>                              
                                                </tr>
                                                ';                                              
											...
											<div class="row">
												<div class="main-content">
													<div class="wrap-content container" id="container" style="width: 100%;height: 100%;">
														<?php 
                       $sqn=mysqli_query($con,"SELECT * FROM tblevents WHERE company='$compp' ORDER BY id DESC");
                       if($sqn){
...
echo '<div class="progress-bar progress-bar-striped bg-info" role="progressbar" style="width:'.number_format($percentage,1).'%;background-color:red;" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">'.number_format($percentage,1).'%</div>';
	...
	<form method="post">
		<div class="modal fade" id="myModalzz" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            ?>
									...
									<option id="myTable" value="<?php echo $row['customer_name'];
	...
	<script type="text/javascript">
		function arChive(id) {
			var id = id;
			if (confirm("Are you sure you want to archive this event?") == true) {
				if (id != '') {
					$.ajax({
						type: 'POST',
						url: 'archive-event.php',
						data: {
							id: id
						},
