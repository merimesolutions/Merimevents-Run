...
if(isset($_POST['submit_comment'])){
	//Redirect if followup person isnt selected
...
$redd=mysqli_query($con,"INSERT INTO customer_comments(customer_comment,customer_person,customer_followup,customer_date,customer_prefdate,customer_id,customer_additional,customer_progress,company,role,followup_user,demo_date,followup_userid,followup_userole) VALUES ('$cust_comment','$cust_person','$cust_followup','$todate','$cust_prefdate','$cust_id','$additional','$progresz','$company','user','$followup_user','$demo_date','$followup_userid','$followup_userole')");
if($redd){
			...
			<section class="content">
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							<form method="post" action="">
								...
								<div class="modal-body" style="height:450px!important; overflow:auto!important;">
									...
									<label class="mb-4 mt-5">Who is responsible for followup? <span class="text-danger">*</span></label>
									...
									while($whamy = mysqli_fetch_array($wham)){ ?>
										<option value="<?php echo $whamy['id']; ?>,User"><?php echo $whamy['full_name']; ?>
										</option>
									...
									while($ban = mysqli_fetch_array($bang)){ ?>
										<option value="<?php echo $ban['id']; ?>,Admin"><?php echo $ban['full_name']; ?>
										</option>
				...
				<div class="row">
					...
					<form method="post">
						...
						<table id="table" class="table table-striped" class="margin-top: 30px!important;">
							<div class="box-body table-responsive">
								...
								<tbody>
									...
									<?php
									$b=1;
							$sdo=mysqli_query($con,"SELECT * FROM customer_comments WHERE (customer_id='$cust_id' AND customer_person='$cust_person') || followup_userid='$cust_person' ");
							while($romm =mysqli_fetch_array($sdo)){
								...
								$sd=mysqli_query($con,"SELECT * FROM tblusers WHERE id='$cutt_person'");
								...
								if(($romm['customer_progress']) == 'In Progress'){
									$dis= '<span style="color:#C76A00; font-family:fangsong; font-size:15px!important;"><strong>In Progress</strong></span> <span class="text-dark" style="font-family:fangsong!important; font-size:15px!important;"><i> ('."$percent".' %)</i></span>';
								}
							...
							//Checking if the followup task is mine or assigned to me
							   ...
							   if(($romm['followup_userid'] !== $cust_person) && ($romm['customer_person'] == $cust_person ) && (!empty($romm['followup_userid']))){
								$taskit ='<span style="color:#FFA500; font-family:fangsong!important; font-size:15px!important;"><strong>'."$personaz".'  Followup </strong></span>';
								...
								?>
									<tr>
										...
										<?php
								//You can only update a followup comment if it's addressed to you or if you added it fot yourself
								if((empty($romm['followup_userid'])) || ($romm['followup_userid'] == $cust_person)){?>
										<td>
										...
										<?php }else{ ?>
										<td>
											<a href="" onclick="return alert('This action is prohibited as the followup is not assigned to you!');" type="submit" name="submit">
