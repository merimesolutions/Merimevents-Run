            ...
            <aside class="right-side">
                ...
                <section class="content-header" style="border-style:;border:1px solid #dedede;">
                    <a  href="javascript:void(0)"  title="Go Back" onclick="goBack()" ><i class="fa fa-angle-double-left" ></i> Back</a>&nbsp;&nbsp;&nbsp;
                     ...
                     <select class="form-select" aria-label="Default select example" style="position: absolute;right: 15px;" id="projects" onchange="showCustomer(this.value)">
                        ...
                        <div class="box-body table-responsive">       
                            <form method="post">
                               <table id="table" class="table  table-striped">
                                        ...
                                        $p=ucwords($row['id']);  
                        $q  = "SELECT tasks.id,  tasks.task_name, tasks.user, tasks.deadline, tasks.date_assigned, tasks.task_progress, tasks.project_id
                        FROM  tasks where  tasks.project_id = '".$p."'
                        ORDER BY id desc";
                        $r = mysqli_query($con, $q);
                            while($rows = mysqli_fetch_array($r))
                                                  { 
                                            $per = $rows['task_progress'];
                                            $d = $rows['deadline'];
                                            if($per<100)
                                            {
                                            ...
                                            echo '
                                                <tr>
                                                <td><small>'.ucwords($rows['task_name']).'</small></td>
