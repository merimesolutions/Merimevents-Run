...
$squeri = mysqli_query($con, "select * from tblusers where company='".$_SESSION['company']."' AND id='$id'");
    ...
    if(isset($_POST['okayadmin2'])){
        ...
        $result = mysqli_query($con,"INSERT INTO singlefaq (user,message,ask_date,username,company,sender_id,rec_id) VALUES ('$user','$faqmessage','$todate','$username','$company','$user', '$id')");
       if($result==true){
							...
							<div class="container overflow-auto col-lg-8 col-md-8 col-sm-8" style="height: 550px;">
								<section class="msger">
									...
									<div id="chatscreen" class="msger-chat vs">
										<?php
  include "../connection.php";
  $sql090=mysqli_query($con,"SELECT * FROM singlefaq WHERE company = '".$_SESSION['company']."' AND (sender_id='$user_id' AND rec_id='$id') OR (rec_id='$user_id' AND sender_id='$id')");
							...
							<div class="col-lg-4 col-sm-12 col-md-4" style="height:550px!important; overflow: auto!important;">
								<section class="msger">
									...
									<div id="chatscreen" class="msger-chat">
										...
										<div class="form-row">
											<div clas="mb-3"><a class="" href="livechat.php"><i class="fas fa-users fa-2x" style="color:#ffad33;font-size: 15px"></i>&nbsp; &nbsp;Group chat</a></div><br>
											<?php
									$squery = mysqli_query($con, "select * from tblusers where company='".$_SESSION['company']."' AND id!='$user_id' AND id!='$id' ORDER BY full_name ");
                                            while($row = mysqli_fetch_array($squery))
                                            {
												$new_id=$row['id'];
												?>
											<div class="mt-2 form-group"><i class="fas fa-user fa-2x " style="color:#00ff55;font-size: 13px;"></i>&nbsp;<span style="font-size: 13px"><a onclick="readChat(<?php echo $new_id; ?> )" href="single_chat.php?id=<?php echo $new_id; ?>"><?php echo $row['full_name']; ?></a></span>
											<?php
						$q = mysqli_query($con,"SELECT * FROM singlefaq where rec_id ='$user_id' and sender_id='$new_id' and reply IS NULL");
                $num_rows = mysqli_num_rows($q);
                if($num_rows>0){
               echo $new_sms = 
                '<span class="badge" style="float:right;background-color:#00802b;"><a style="color:#fff !important;" href="single_chat.php?id='.$row['id'].'" onClick="readChat('.$row['id'].')">'.$num_rows.'</a></span>';
              }else{
               echo $new_sms = '';
              }  
											?></div>
