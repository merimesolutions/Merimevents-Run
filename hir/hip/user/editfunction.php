...
<?php
	if(isset($_POST['btn_progress']))
	{
        ...
        $comment_edit_sy = mysqli_real_escape_string($con,$_POST['comment_edit_sy']);
        $user=$_SESSION['userid'];
	    $todate=date("Y-m-d h:i:sa");
	    $who ="user";
	    $task = $_GET['task'];
	    $reply = 1;
	    $query = mysqli_query($con,"UPDATE tasks SET task_progress = '".$progress_edit_sy."' where id = '".$txt_id."' ");
	    if (!empty($comment_edit_sy)) {
	    $result = mysqli_query($con,"INSERT INTO faq (user,message,ask_date,who,task) VALUES ('$user','$comment_edit_sy','$todate','$who','$task')");
		...
		if(mysqli_error($con)){
			echo '<script type="text/javascript">'; 
            echo 'alert("No changes made.");'; 
            echo 'window.location.href = "pl.php";';
            echo '</script>';
		}
	}
?>
...
<?php
	if(isset($_POST['btn_pay']))
	{
        ...
        $q = mysqli_query($con,"INSERT INTO tbltransactions(random,invoice,amount,initial,date_paid, served_by,trans,person,person_id) values ('".$txt_id."','".$invoice."','".$amount."','".$new."','".$date."','".$editor."','".$trans."','".$person."','".$person_id."' )");
		...
		if(mysqli_error($con)){
			echo '<script type="text/javascript">'; 
            echo 'alert("No changes made.");'; 
            echo 'window.location.href = "ilp.php";';
            echo '</script>';
		}
	}
?>