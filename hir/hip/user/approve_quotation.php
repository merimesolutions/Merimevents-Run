...
include "../connection.php";
 ...
 if($number > 0){
     echo '<script>alert("This quotation had already been previously approved!");window.location="view_event.php?id='."$event_id".'";</script>';
 }
 }
 ...
 while($roz =mysqli_fetch_array($mez)){
     ...
     if($sql){
         ...
         $f=mysqli_query($con,"SELECT * FROM event_quotation WHERE event_id='$event_id'");
         ...
         echo '<script>alert("Quotation succesfully Approved");window.location="view_event.php?id='."$event_id".'";</script>';
     }else{
         $error =mysqli_error($con);
           echo '<script>alert(" '."$error".'");window.location="view_event.php?id='."$event_id".'";</script>';
     }
}
 }
 ?>