...
if (!isset($_SESSION['userid'])){
 ...
 if(isset($_POST['btn_add_cutomer'])){
     ...
     if($number_pe > 0){
         echo '<script>alert("This customer record already exists!!");window.location="add.php";</script>';
     }else{
         $sl=mysqli_query($con,"INSERT INTO tblcustomers (fname,email,fcontact,customer_name,identity,company) VALUES ('$new_name', '$new_email', '$new_phone', '$customer_name', '$identity','".$_SESSION['company']."')");
         if($sl){
             echo'<script>window.location.href = window.location.href;</script>';
         }else{
             $errp=mysqli_error($con);
             echo'<script>alert(" '."$errp".'");window.location="add.php";</script>'; 
         }
     }
 }
 //Comprehensive Customers
  ...
  $sql_u =mysqli_query($con, "SELECT * FROM tblcustomers WHERE identity = '$identity' and company='$company' ");
        if($sql_u){
            ...
            if($numbb > 0){
            echo '<script>alert("This customer record already exists in the system!");window.location="add.php";</script>';
    }else{
          ...
          if($sql){
                echo '<script>alert("Customer Registered succesfully");window.location="add.php";</script>';
          } else {
        
        echo '<script>alert("An error occured during submission");window.location="add.php";</script>';
          }
                                    ...
                                    <table class="table table-striped" id="table_field">
                                        ...
                                        $company=$_SESSION['company'];
                                            ...
                                            foreach($txtItem_name as $key => $value){
                                                $time1 = strtotime($txtRdate[$key]);
                                            $dif   = floor( ($time1-$time2) /(60*60*24));
                                                $total_cost = $dif * $txtQnty[$key] * $txtbal_Price[$key];
